...
const { initialized, isAuthenticated, canWrite } = useAuth();
const { unreadCount, refreshUnreadCount } = useNotificationState();
watch(isAuthenticated, (active) => { if (active) refreshUnreadCount(); else unreadCount.value = 0; }, { immediate: true });
</script>
...
<template>
  ...
  <div v-else-if="isAuthenticated" class="app-shell">
    ...
    <header class="topbar">
      ...
      <nav aria-label="主导航"><RouterLink to="/"><Sparkles :size="16" /> 首页</RouterLink><RouterLink to="/explore"><Compass :size="16" /> 发现</RouterLink><RouterLink to="/collections"><LibraryBig :size="16" /> 专题</RouterLink><RouterLink to="/library"><BookMarked :size="16" /> 知识库</RouterLink><RouterLink to="/subscriptions"><Rss :size="16" /> 订阅</RouterLink><RouterLink v-if="canWrite" to="/articles"><Files :size="16" /> 创作中心</RouterLink></nav>
      <div class="topbar-actions"><AccountMenu /><RouterLink class="notification-shortcut" to="/notifications" aria-label="通知中心"><Bell :size="17" /><span v-if="unreadCount" class="notification-badge">{{ unreadCount > 99 ? "99+" : unreadCount }}</span></RouterLink><RouterLink v-if="canWrite" class="write-shortcut" to="/articles/new"><FilePenLine :size="16" /><span>写文章</span></RouterLink></div>
    </header>
