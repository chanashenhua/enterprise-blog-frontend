...
const STORAGE_KEY = "enterprise-blog.portal.auth.v1";
...
function resolveUserId(input: unknown): DemoUserId | null {
  const candidate = typeof input === "string"
    ? input
    : typeof input === "object" && input !== null && "userId" in input
...
export class LocalDemoAuthProvider implements AuthProvider {
  ...
  constructor(
    ...
    private readonly mockToken: string | undefined,
  ) {}

  async initialize(): Promise<AuthSession | null> {
    if (!this.mockToken?.trim()) {
  ...
  async login(input: unknown): Promise<AuthSession> {
    if (!this.mockToken?.trim()) {
      throw new Error("本地演示登录未配置：请设置 VITE_MOCK_OIDC_TOKEN=local-dev-token");
    }
  ...
  async authorizationHeaders(): Promise<Record<string, string>> {
    ...
    return {
      "X-Mock-Token": this.mockToken,
      "X-Mock-User": user.id,
      "X-Mock-Roles": user.roles.join(","),
      "X-Mock-Departments": user.departmentIds.join(","),
      "X-Mock-Teams": user.teamIds.join(","),
    };
  }
}
...
class OidcAuthProviderPlaceholder implements AuthProvider {
  ...
  async login(): Promise<AuthSession> {
    throw new Error("企业 OIDC 尚未接入，请先配置 Issuer、Client ID 和回调地址");
  }
  async logout() {}
  async authorizationHeaders(): Promise<Record<string, string>> {
...
function createAuthProvider(): AuthProvider {
  ...
  const localMode = import.meta.env.DEV && configuredMode !== "oidc";
...
export async function login(userId: DemoUserId): Promise<AuthSession> {
  ...
  try {
    const next = await provider.login({ userId });
    session.value = next;
    return next;
  } catch (reason) {
    authError.value = reason instanceof Error ? reason.message : "登录失败";
    throw reason;
  }
}
